...
router.post('/register', async(req, res, next)=>{
 try{
  ...
  if(existingUser){
   return res.status(400).json(createResponse(false, 'Email alreaday exist'));
  }
  ...
  res.status(201).json(createResponse(true,'User Registered siccessfully'));
 }
 catch(err){
  next(err)
 }
...
router.post('/login', async(req, res, next)=>{
 ...
 if(!user){
  return res.status(400).json(createResponse(false, ' email Invalid credentials'));
 }
